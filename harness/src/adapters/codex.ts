...
export class CodexAdapter implements AgentAdapter {
  ...
  async *execute(req: AgentAdapterRequest): AsyncIterable<AgentEvent> {
    const result = await this.runProcess(
      ['--quiet', '--approval-mode', 'full-auto', req.prompt],
  ...
  private runProcess(args: string[], cwd: string): Promise<{ output: string; success: boolean; error: string }> {
    return new Promise((resolve) => {
      ...
      proc.on('close', (code) => {
        this.currentProcess = null;
        if (code !== 0) {
          resolve({ output: '', success: false, error: `codex exited with code ${code}: ${stderr || stdout}` });
        } else {
      ...
      proc.on('error', (err) => {
        this.currentProcess = null;
        resolve({ output: '', success: false, error: `failed to spawn codex: ${err.message}` });
      });
    });
  }
}
