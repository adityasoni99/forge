...
- [Anthropic — Scaling Managed Agents: Decoupling the brain from the hands](https://www.anthropic.com/engineering/managed-agents) — Brain-hands-session decoupling, containers as cattle, lazy provisioning (TTFT), credential isolation, durable session log as external context object, capability-aware interfaces that outlast harness assumptions.
- [Claude Managed Agents platform docs](https://platform.claude.com/docs/en/managed-agents/overview) — Agent/Environment/Session/Events model, built-in tools, SSE streaming, multi-agent research preview.
